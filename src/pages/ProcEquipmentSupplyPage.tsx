...
// Mock previous-month closing balances per item to simulate auto-population
...
const SEED: EquipRecord[] = [
{
  id: 1,
  reportDate: '2026-04-01',
  itemDescription: 'MPMO Laptops',
  openingBalance: 18,
  qtyReceived: 5,
  shipmentNumber: '78214',
  shipmentDoc: 'PTR-MPMO-LAP-04-2026.pdf',
  dateReceivedEndUser: '2026-04-08',
  qtyDefects: 1,
  qtyDefectsDoc: 'Defect-Report-MPMO-Apr.pdf',
  closingBalance: 22,
  remarks: 'One unit had a faulty keyboard; flagged for replacement.',
  inventoryReportDoc: 'Manual-Inventory-Apr-2026.pdf',
  isDraft: false
},
{
  id: 2,
  reportDate: '2026-04-01',
  itemDescription: 'Tablets',
  openingBalance: 40,
  qtyReceived: 10,
  shipmentNumber: '78215',
  shipmentDoc: '',
  dateReceivedEndUser: '2026-04-10',
  qtyDefects: 0,
  qtyDefectsDoc: '',
  closingBalance: 50,
  remarks: 'All tablets received in good condition.',
  inventoryReportDoc: '',
  isDraft: false
},
{
...
function loadRecords(): EquipRecord[] {
  return storageGet<EquipRecord[]>(KEYS.PROC_EQUIP_SUPPLY, SEED);
}
function saveRecords(records: EquipRecord[]) {
  storageSet(KEYS.PROC_EQUIP_SUPPLY, records);
}
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Equipment Supply</h1>
          <p className="text-sm text-gray-500">
            Track IT and office equipment inventory at the municipality level
          </p>
        </div>
...
function EquipForm({
...
}: {onSubmit: (r: Omit<EquipRecord, 'id'>) => void;}) {
  ...
  const closingBalance = useMemo(() => {
    return form.openingBalance + form.qtyReceived - form.qtyDefects;
  }, [form.openingBalance, form.qtyReceived, form.qtyDefects]);
  const updateField = (field: string, value: string | number) => {
    setForm((f) => {
      ...
      // Auto-populate opening balance from previous closing when item is chosen
        ...
        <div className="flex items-center gap-3 px-5 sm:px-6 pt-5 sm:pt-6 pb-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              Select the equipment to report on
            </p>
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="flex items-center gap-3 px-5 sm:px-6 pt-5 sm:pt-6 pb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Opening allocation and end-of-month balance
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 sm:p-6 pt-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Auto-calculated: Opening + Received − Defects
                  </p>
                  {closingBalance < 0 &&
              ...
              <div className="space-y-4 p-5 sm:p-6 pt-4">
                ...
                <motion.div
                  ...
                  className="overflow-hidden">
                    ...
                    helperText="Upload a supporting document for defective items" />
...
}: {records: EquipRecord[];searchQuery: string;onSearch: (q: string) => void;onDelete: (id: number) => void;}) {
  const [page, setPage] = useState(1);
  const [viewRecord, setViewRecord] = useState<EquipRecord | null>(null);
  const perPage = 5;
  const totalPages = Math.max(1, Math.ceil(records.length / perPage));
  const paginated = records.slice((page - 1) * perPage, page * perPage);
  function getViewSections(r: EquipRecord): SectionDef[] {
    return [
    {
              ...
              className="border-b border-gray-50 hover:bg-gradient-to-r hover:from-primary-50/30 hover:to-transparent hover:shadow-sm transition-all duration-200 cursor-pointer group">
                ...
                <td className="px-4 py-3.5">
                  <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                    {/* Updated here: removed delete button */}
                    {/* Modified the actions container to only show view icon */}
                    <button
                    ...
                    {/* Removed delete button per comment */}
                  </div>
                </td>
              </tr>
            )}
        ...
        <div
          ...
          className="border border-gray-100 rounded-xl p-4 space-y-2 hover:border-primary/30 hover:shadow-md hover:shadow-primary/5 transition-all duration-200 cursor-pointer group relative overflow-hidden">
            ...
            <div className="flex items-center justify-end pt-1">
              <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                {/* Updated here: only show view icon, no delete */}
                <button
                onClick={() => setViewRecord(r)}
